...
class WebAudioRecorder {
        ...
        try {
          if (_recordedChunks.isNotEmpty) {
            ...
            reader.onError.listen((event) {
              _logger.severe('Error reading audio data: $event');
  ...
  /// Stop recording and transcribe (stub - transcription handled separately)
  Future<String?> stopRecordingAndTranscribe() async {
    final audioData = await stopRecording();
    if (audioData != null) {
      // Audio data is available, but transcription is handled by STT service
      _logger.info(
        'Audio recorded successfully, length: ${audioData.length} bytes',
      );
      return 'Audio recorded successfully'; // Placeholder
    }
    return null;
  }

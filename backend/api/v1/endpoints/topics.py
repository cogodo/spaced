...
async def get_review_statistics(
    ...
    try:
        ...
        if average_retention < 0.7:
            insights.append("Consider reviewing topics more frequently to improve retention")
        
        if weekly_reviews > total_topics * 0.5:
            insights.append("Heavy review load this week - consider spreading out study sessions")
        
        overdue_count = len([t for t in topics if t.nextReviewAt and t.nextReviewAt < current_time])
        if overdue_count > 0:
            insights.append(f"{overdue_count} topics are overdue for review")

    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        if self.start_time:
            ...
            if duration >= self.timeout_seconds:
                self.timed_out = True
                logger.warning(f"Operation '{self.operation}' timed out",
                             operation=self.operation,
                             duration=duration,
                             timeout=self.timeout_seconds)
                
                increment_counter("operation_timeout",
                                {"operation": self.operation})
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if duration >= timeout_seconds:
                    logger.warning(f"Function '{op_name}' exceeded timeout but completed",
                                 function=func.__name__,
                                 duration=duration,
                                 timeout=timeout_seconds)
                    
                    increment_counter("function_timeout_exceeded",
                                    {"function": func.__name__})
                ...
                if duration >= timeout_seconds:
                    logger.error(f"Function '{op_name}' timed out and failed",
                               function=func.__name__,
                               duration=duration,
                               timeout=timeout_seconds,
                               error_type=type(e).__name__)
                    
                    increment_counter("function_timeout_with_error",
                                    {"function": func.__name__})
...
async def async_with_timeout(coro, timeout_seconds: float, operation: str = "async_operation"):
    ...
    try:
        ...
        logger.warning(f"Async operation '{operation}' timed out",
                     operation=operation,
                     timeout=timeout_seconds)
        
        increment_counter("async_operation_timeout",
                        {"operation": operation})
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs):
            try:
                ...
                logger.error(f"Async function '{op_name}' failed",
                           function=func.__name__,
                           error_type=type(e).__name__,
                           error_message=str(e))
                raise
...
def adaptive_timeout(previous_durations: list, multiplier: float = 1.5, min_timeout: float = 1.0, max_timeout: float = 300.0) -> float:
    """

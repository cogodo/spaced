...
class MetricsCollector:
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        with self._lock:
            # API request metrics
            total_requests = sum(
                counter.value for key, counter in self._counters.items()
            ...
            error_requests = sum(
                counter.value for key, counter in self._counters.items()
                if counter.name == "api_requests_total" and 
                counter.tags.get("status", "").startswith(("4", "5"))
            )
            ...
            return {
                ...
                "avg_response_time_ms": round(api_duration_hist.average() * 1000, 2) if api_duration_hist and api_duration_hist.values else 0,
                "p95_response_time_ms": round(api_duration_hist.percentile(0.95) * 1000, 2) if api_duration_hist and api_duration_hist.values else 0,
                "active_sessions": self._gauges.get("active_sessions", GaugeMetric("active_sessions")).value,
                "total_sessions_started": self._counters.get("sessions_started_total", CounterMetric("sessions_started_total")).value,
                "total_questions_answered": self._counters.get("questions_answered_total", CounterMetric("questions_answered_total")).value,
            }

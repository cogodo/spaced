...
from core.models import Question, Session, Topic
...
class StructureTest:
    ...
    async def _test_question_operations(self, topic: Topic):
        ...
        for i in range(5):
            question = Question(
                ...
                topicId=topic.id,
                text=(
                    f"Test question {i+1}: What is a Python "
                    f"{['variable', 'function', 'class', 'module', 'package'][i]}?"
                ),
                type="short_answer",
                difficulty=1,
                metadata={"test": True},
    ...
    async def _test_session_operations(self, topic: Topic):
        ...
        (
            retrieved_session,
            current_question,
        ) = await self.session_service.get_current_question(session.id, self.test_user_uid)
    ...
    async def cleanup_test_data(self):
        ...
        try:
            ...
            for topic in topics:
                ...
                # Get all user sessions and delete them - use topic service to
                # get topics with sessions
                topics = await self.topic_service.get_user_topics(self.test_user_uid)
                for topic in topics:
                    sessions = await self.session_service.get_sessions_for_topic(self.test_user_uid, topic.id)

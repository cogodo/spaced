...
from core.models import Topic, Question, Session, FSRSParams
...
class FSRSSystemTest:
    ...
    async def run_fsrs_tests(self):
        ...
        try:
            ...
            questions = await self._create_test_questions(topic)
            
            # Test 2: Run learning session with good performance
            session = await self._run_learning_session(topic, good_performance=True)
    ...
    async def _run_learning_session(self, topic: Topic, good_performance: bool = True):
        ...
        logger.info("Running learning session with %s performance...", 
        ...
        for i in range(5):
            ...
            _, current_question = await self.session_service.get_current_question(
                session.id, self.test_user_uid
            )
    ...
    async def _test_review_endpoints(self):
        ...
        from api.v1.endpoints.topics import get_topics_with_review_status, get_todays_reviews, get_review_statistics
        ...
        logger.info("Due today: %d, Overdue: %d", 
                   len(todays_reviews["dueToday"]), len(todays_reviews["overdue"]))
        ...
        logger.info("Study statistics - Topics: %d, Retention: %.2f", 
                   stats["totalTopics"], stats["averageRetention"])

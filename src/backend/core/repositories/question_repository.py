...
class QuestionRepository:
    ...
    async def create(self, question: Question, user_uid: str) -> Question:
        ...
        doc_ref = self._get_topic_questions_collection(
            user_uid, question.topicId
    ...
    async def get_by_id(
        ...
        doc = (
            self._get_topic_questions_collection(user_uid, topic_id)
    ...
    async def list_by_topic(
        self, topic_id: str, user_uid: str, limit: int = 50
    ) -> List[Question]:
    ...
    async def update(
        self, question_id: str, user_uid: str, topic_id: str, updates: dict
    ) -> None:
        """Update question fields in topic's subcollection"""
        doc_ref = self._get_topic_questions_collection(user_uid, topic_id).document(
            question_id
        )
    ...
    async def delete(self, question_id: str, user_uid: str, topic_id: str) -> None:
        ...
        self._get_topic_questions_collection(user_uid, topic_id).document(
            question_id
        ).delete()

...
class SessionRepository:
    ...
    async def create(self, session: Session) -> Session:
        ...
        fields
        """
        doc_ref = self._get_user_sessions_collection(session.userUid).document(
            session.id
        )
        ...
        if existing_doc.exists:
            ...
            print(
                f"DEBUG: Updated existing session {session.id} with backend data, "
                f"set token to session ID"
            )
        else:
    ...
    async def get_by_id(self, session_id: str, user_uid: str) -> Optional[Session]:
        ...
        if doc.exists:
            data = doc.to_dict()
            return Session.model_validate_dict(
                data, doc_id=session_id, user_uid=user_uid
            )
        return None
    ...
    async def list_by_user(self, user_uid: str) -> List[Session]:
        ...
        for doc in docs:
            data = doc.to_dict()
            sessions.append(
                Session.model_validate_dict(data, doc_id=doc.id, user_uid=user_uid)
            )

        return sessions

    async def list_by_user_and_topic(
        self, user_uid: str, topic_id: str
    ) -> List[Session]:
        """Get sessions for a user and specific topic"""
        query = self._get_user_sessions_collection(user_uid).where(
    ...
    async def add_message(
        ...
        doc_ref = self._get_session_messages_collection(user_uid, session_id).document(
            message.id
        )
    ...
    async def get_session_messages(
        self, user_uid: str, session_id: str, limit: int = 30
    ) -> List[Message]:
        """Get messages for a session, ordered by timestamp"""
        query = (
            self._get_session_messages_collection(user_uid, session_id)
            .order_by("timestamp")
            .limit(limit)
        )
